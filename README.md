...
3. **Which models?** The CLI's `initialize` handshake returns the account's own picker (verified through the admission relay: zero upstream requests). Every row keeps its own Hermes route id: `opus` (the included 200K window) and `opus[1m]` (the 1M window, metered separately) are two rows, never collapsed into one. Rows the CLI marks "Draws from usage credits", plus Fable on non-Max plans per Anthropic's plan rule, carry a dim `· usage credits` note on that row alone; nothing is hidden. If the handshake fails the pinned catalog below is used.
...
The pinned picker lists one row per explicit native route, base and `[1m]` alike, the way the CLI's own picker does. Each row selects exactly itself on the native command line:

| Model | Native selection | Context |
| --- | --- | --- |
| Sonnet 5 | `claude-sonnet-5` / `claude-sonnet-5[1m]` | 200,000 / 1,000,000 |
| Haiku 4.5 | `claude-haiku-4-5-20251001` | 200,000 |
| Opus 5 | `claude-opus-5` / `claude-opus-5[1m]` | 200,000 / 1,000,000 |
| Opus 4.8 | `claude-opus-4-8` / `claude-opus-4-8[1m]` | 200,000 / 1,000,000 |
| Fable 5.1 | `claude-fable-5-1` / `claude-fable-5-1[1m]` | 200,000 / 1,000,000 |

Short names `sonnet`, `haiku`, `opus` and `fable` resolve to the version-pinned base ids above, never to a `[1m]` route. Unknown model IDs pass through unchanged with no invented context metadata. An explicit Hermes `model.context_length` still overrides the host's window, including a smaller compaction budget.

### Context routing

The local relay sets `ANTHROPIC_BASE_URL`, which makes Claude Code apply its gateway defaults: a 1M-capable model runs its included 200K window unless `[1m]` is selected on the command line ([Claude Code model configuration](https://code.claude.com/docs/en/model-config#sonnet-5-context-window)). The `[1m]` route is metered differently from included usage on most plans (the native picker marks Opus 1M and Fable as drawing from usage credits), so the plugin never appends the suffix silently. An earlier release did, which routed every ordinary `sonnet`/`opus` conversation to the 1M route; that is the context/billing bug this policy replaces.

`CLAUDE_SUBSCRIPTION_DIRECTSDK_CONTEXT_ROUTING` (or `Client(context_routing=...)`, which wins) selects the policy:

| Policy | Ordinary id or alias (`sonnet`, `claude-opus-5`) | Explicit `[1m]` id | Hermes window |
| --- | --- | --- | --- |
| `auto` (default) | included 200K route first; one retry on `[1m]` if the API rejects the request as over the window | `[1m]` from the first request | 200K for ordinary ids, 1M for `[1m]` ids |
| `always-1m` (`1m`) | `[1m]` for every 1M-capable model; Haiku stays 200K | `[1m]` | 1M (Haiku 200K) |
| `always-200k` (`200k`) | base route | base route, suffix ignored | 200K |

Under `auto` a short conversation never leaves the included route, and Hermes budgets and compacts against 200K. The escalation is a safety net, not a standing budget: it fires only for a *genuine* context-window failure reported by the API, the pre-generation `prompt is too long` / `exceed context limit` rejection, or a `model_context_window_exceeded` stop, and only while nothing has reached the caller yet. A stream that already delivered text is returned with `finish_reason: length` rather than re-run, because the same request cannot be replayed into a consumer that has seen half of it. The retried request is a fresh native spawn with its own admission; `usage.native_admission` reports `routes` (every native selection tried, in order), `route` (the one that answered) and `upstream_requests` summed over the attempts, so an escalation is visible in Hermes accounting rather than folded into a single request. A misspelt policy fails the request with its own message; it never picks a route by guess. Haiku has no 1M route: `always-1m` keeps it on 200K and `haiku[1m]` is rejected rather than downgraded.
...
Transport invariant tests cover signed replay and harmless normalization, transformed projections, final tool batches/usage, async use, lazy failure, invalid parameters, conflicting auth, and active/paused/unstarted stream cleanup. Context-routing tests drive a fake native through the admission relay against a loopback API peer and check the native `--model` argv per spawn: ordinary ids and aliases select their base route under every policy except `always-1m`, `auto` retries a `prompt is too long` rejection (and an unseen `model_context_window_exceeded` stop) once on `[1m]` with both routes recorded in `usage.native_admission`, a streamed truncation is never re-run, `always-200k` surfaces the rejection without a retry, and a misspelt policy fails before any spawn. Discovery tests assert that the live picker keeps base and `[1m]` rows distinct with each row's own `usage credits` note. The admission regression fails on the previous implementation (two upstream requests) and passes with one request, preserving first-response usage including zero values. Its cancellation control verifies upstream socket closure. A real-native ten-case loopback qualification covers normal text, tools, output/context limits, thinking-only recovery, refusal, HTTP errors, disconnects and cancellation, with one upstream request per call. Its responses are synthetic protocol fixtures, not paid-model evidence.
